#' Univariate Slice Sampler using Stepping-out Method and Shrinkage
#'
#' This function implements a slice sampler using the stepping-out and shrinkage
#' methods.
#'
...
#'   state <- univariate_slice_sampler_stepping_out(state, target, FALSE, 1)$value
#'   sum <- sum + state
#' }
#' sum / n_samples
#' 
...
#' 
univariate_slice_sampler_stepping_out <- function(x, target, on_log_scale, width) {
  ...
  .Call(.univariate_slice_sampler_stepping_out, x, target, on_log_scale, width)
}
